...
![Fig 5.1: 트랜잭션 ID와 튜플](../docs/images/ch05/fig-5-01.png)
*트랜잭션 ID와 튜플*

> **🔍 그림 해설**
>
> 모든 트랜잭션은 식당에서 번호표를 받는 것처럼 고유한 번호를 받습니다. 이 번호가 바로 트랜잭션 ID(XID)입니다.
> 당신이 데이터 행(튜플)을 INSERT하면, 당신의 번호표가 그 행의 "xmin" 필드에 도장처럼 찍힙니다. "이 행은 1000번 손님이 만들었어!"
> 나중에 누군가 그 행을 DELETE하면, 그 사람의 번호표가 "xmax" 필드에 찍힙니다. "이 행은 1005번 손님이 삭제했어!"
> xmax가 0이면? "아무도 삭제 안 했어, 아직 살아있어!" PostgreSQL은 이 두 개의 번호표(xmin, xmax)만 보고
> "이 행이 지금 당신 눈에 보여야 하는가?"를 판단합니다. 매우 간단하지만 강력한 메커니즘이죠.
...
![Fig 5.2: 튜플 INSERT/DELETE/UPDATE 시 xmin/xmax 변화](../docs/images/ch05/fig-5-02.png)
*튜플 INSERT/DELETE/UPDATE 시 xmin/xmax 변화*

> **🔍 그림 해설**
>
> PostgreSQL의 UPDATE는 마치 연필로 지우고 다시 쓰는 게 아니라, 기존 내용 위에 취소선을 긋고 옆에 새로 쓰는 것과 같습니다.
> INSERT할 때: 새 행이 생기고 xmin에 내 번호표가 찍힙니다. xmax는 0 (아직 아무도 안 죽였어).
> DELETE할 때: 행을 물리적으로 지우지 않고, xmax 필드에 내 번호표를 찍어 "나 이거 죽였어!"라고 표시합니다.
> UPDATE할 때가 흥미로운데, 실제로는 DELETE(기존 행의 xmax 표시) + INSERT(새 행 생성)가 동시에 일어납니다.
> 그래서 UPDATE 한 번이면 실제로는 파일에 두 개 버전의 행이 모두 존재하게 됩니다. 이것이 MVCC의 핵심입니다.

![Fig 5.3: MVCC 튜플 버전 관리](../docs/images/ch05/fig-5-03.png)
*MVCC 튜플 버전 관리*

> **🔍 그림 해설**
>
> 하나의 행에 대해 여러 버전이 동시에 존재할 수 있습니다. 마치 사진첩에 같은 장소를 다른 시간에 찍은 사진들이 함께 있는 것과 같죠.
> 당신이 트랜잭션을 시작하면 "스냅샷 타임스탬프"를 받습니다. 이건 마치 "2024년 1월 31일 10시에 찍은 사진만 보겠어"라고 선언하는 것입니다.
> 다른 사람이 같은 행을 UPDATE해서 새 버전을 만들어도, 당신은 당신의 타임스탬프 시점의 버전만 봅니다.
> 다른 트랜잭션은 다른 타임스탬프를 가지고 있어서 더 최신 버전을 볼 수 있죠. 이렇게 각자가 일관된 자기만의 뷰를 보면서,
> 서로 차단하지 않고 동시에 작업할 수 있습니다. 이것이 MVCC의 마법입니다 - 읽기와 쓰기가 서로를 차단하지 않습니다!
...
![Fig 5.4: Commit Log (CLOG) 구조](../docs/images/ch05/fig-5-04.png)
*Commit Log (CLOG) 구조*

> **🔍 그림 해설**
>
> CLOG는 PostgreSQL의 "출석부"입니다. 모든 트랜잭션 번호표(XID)에 대해 2비트짜리 상태가 기록됩니다.
> "진행 중(IN_PROGRESS)", "커밋됨(COMMITTED)", "취소됨(ABORTED)" 중 하나죠. pg_xact/ 디렉토리에 파일로 저장됩니다.
> 왜 필요할까요? 행의 xmin을 봤을 때 "이 행을 만든 트랜잭션이 정말 성공했어?"를 확인해야 합니다.
> CLOG를 보고 "오, 1000번 트랜잭션은 COMMITTED 상태네. 그럼 이 행 보여줘도 돼"라고 판단합니다.
> 만약 ABORTED 상태면? "이 행은 실패한 트랜잭션이 만든 유령이야. 아무에게도 보이면 안 돼!"
...
![Fig 5.6: 락 매니저 구조](../docs/images/ch05/fig-5-06.png)
*락 매니저 구조*

> **🔍 그림 해설**
>
> 락 매니저는 도서관의 대출 시스템과 같습니다. 공유 메모리에 해시 테이블 형태로 "누가 어떤 책을 빌렸는지" 기록합니다.
> 여러 사람이 같은 책을 동시에 읽을 수 있습니다(shared lock - 공유 락). 하지만 누군가 그 책에 필기하려면(exclusive lock - 배타적 락),
> 다른 사람들이 모두 반납할 때까지 기다려야 합니다. 락 테이블은 이 모든 예약 상태를 추적하는 사서의 장부입니다.
> AccessShare, RowExclusive 등 여러 락 모드가 있는데, 이들은 "읽기만", "쓰기만", "구조 변경" 등 다양한 접근 수준을 표현합니다.
> OS 파일 락 대신 메모리에서 관리하는 이유는? 훨씬 빠르고, 데드락 감지도 할 수 있고, 행 단위 세밀한 제어가 가능하기 때문입니다.
...
![Fig 5.9: 데드락 감지](../docs/images/ch05/fig-5-09.png)
*데드락 감지*

> **🔍 그림 해설**
>
> 데드락은 좁은 복도에서 두 사람이 서로를 막고 있는 상황입니다. "너 비켜!" "아니 네가 먼저 비켜!"
> 트랜잭션 1이 A 락을 잡고 B 락을 기다리고, 동시에 트랜잭션 2가 B 락을 잡고 A 락을 기다리면 사이클이 형성됩니다.
> PostgreSQL은 1초마다 "데드락 탐정"을 돌립니다. 락 대기 그래프에서 사이클을 찾아내죠.
> 사이클이 발견되면? 한 명을 희생양으로 선택해서 강제로 ROLLBACK 시킵니다. 그러면 다른 트랜잭션이 진행할 수 있게 됩니다.
> 희생된 트랜잭션은 에러를 받고, 애플리케이션에서 재시도해야 합니다. 데드락을 막으려면? 모든 트랜잭션이 같은 순서로 자원을 접근하면 됩니다.
