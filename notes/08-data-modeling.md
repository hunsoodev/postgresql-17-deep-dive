...
![Fig 1.1: 데이터베이스 클러스터 논리적 구조](../docs/images/ch01/fig-1-01.png)
*Database cluster logical structure: Cluster → Databases → Schema objects (tables, indexes, views).*

> **🔍 그림 해설**
>
> PostgreSQL의 최상위 개념인 "클러스터"는 하나의 데이터 디렉토리에 여러 데이터베이스를 담는 그릇입니다. 대학 캠퍼스(클러스터)에 여러 건물(데이터베이스)이 있고, 각 건물에 강의실(테이블), 복도(인덱스), 창문(뷰)이 있는 것과 같습니다. 하나의 클러스터는 하나의 PostgreSQL 서버 인스턴스를 의미하며, 같은 포트를 공유합니다. 각 데이터베이스는 논리적으로 완전히 분리되어 있어 서로의 테이블에 직접 접근할 수 없습니다(dblink 제외). 스키마는 데이터베이스 내의 네임스페이스로, 같은 이름의 테이블을 다른 스키마에 만들 수 있습니다. 이 계층 구조를 이해하면 백업, 권한 관리, 연결 문자열 설정 등이 명확해집니다.
