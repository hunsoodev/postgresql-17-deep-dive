...
![Fig 9.1: WAL 쓰기 전체 흐름](../docs/images/ch09/fig-9-01.png)
*WAL 쓰기 전체 흐름*

> **🔍 그림 해설**
>
> WAL을 마트 계산대에 비유해 봅시다. 여러분이 계산원이고, 큰 장부(데이터 파일)와 영수증 테이프(WAL)가 있다고 상상하세요. 손님이 물건을 사면(트랜잭션), 두 가지를 해야 합니다. 먼저 영수증 테이프에 재빨리 적습니다(WAL 기록). 이건 빠릅니다. 테이프 끝에 쭉쭉 적기만 하면 되니까요. 그림에서 보듯이 트랜잭션이 커밋되면 WAL 레코드가 먼저 디스크에 fsync됩니다. 큰 장부(데이터 파일)는 나중에 여유 있을 때 정리합니다. 이때 하는 것이 checkpoint입니다. 만약 가게가 갑자기 정전되면 어떻게 될까요? 큰 장부는 중간까지만 쓰여 있을 수 있지만, 영수증 테이프만 있으면 거기서부터 다시 복원할 수 있습니다. 이것이 WAL의 핵심 아이디어입니다. 빠른 순차 쓰기로 일단 안전하게 기록하고, 느린 랜덤 쓰기는 나중에 여유롭게 하는 것이죠. 그림의 화살표를 따라가면 이 흐름이 명확히 보일 것입니다.
...
![Fig 9.2: WAL 버퍼와 WAL 세그먼트 관계](../docs/images/ch09/fig-9-02.png)
*WAL 버퍼와 WAL 세그먼트 관계*

> **🔍 그림 해설**
>
> WAL buffer는 메모리에 있는 작은 메모장이고, WAL 파일은 디스크에 있는 큰 공책입니다. 트랜잭션이 데이터를 변경하면 일단 메모리의 WAL buffer에 재빨리 적습니다. 메모리는 엄청 빠르니까요. 하지만 메모리는 휘발성입니다. 정전되면 날아가죠. 그래서 주기적으로, 그리고 커밋할 때마다 WAL buffer의 내용을 디스크의 WAL 파일로 옮깁니다. 이 작업을 하는 것이 WAL writer 프로세스입니다. pg_wal/ 디렉토리를 보면 000000010000000000000001, 000000010000000000000002 같은 16MB 파일들이 계속 생깁니다. 이것들이 WAL 세그먼트 파일입니다. 마치 공책이 다 차면 새 공책을 쓰는 것처럼, 16MB가 차면 다음 파일로 넘어갑니다. 그림에서 WAL buffer가 작은 상자로, WAL 파일들이 큰 실린더로 표현된 것을 볼 수 있습니다. 이 구조 덕분에 빠른 메모리 쓰기와 안전한 디스크 저장을 동시에 달성합니다.
...
![Fig 9.5: 체크포인트 처리 과정](../docs/images/ch09/fig-9-05.png)
*체크포인트 처리 과정*

> **🔍 그림 해설**
>
> Checkpoint는 게임을 저장하는 것과 같습니다. 게임을 하다가 중간중간 저장 버튼을 누르죠? PostgreSQL도 마찬가지입니다. 메모리(shared_buffers)에는 수정했지만 아직 디스크에 안 쓴 "더티 페이지"들이 쌓여 있습니다. Checkpoint가 시작되면 checkpointer 프로세스가 이 더티 페이지들을 전부 디스크에 씁니다. 단, 한꺼번에 확 쓰면 시스템이 버벅거리므로, checkpoint_completion_target 설정(보통 0.9)에 따라 90% 시간에 걸쳐 천천히 씁니다. 예를 들어 checkpoint_timeout이 5분이면, 4.5분에 걸쳐 조금씩 쓰는 거죠. 다 쓰고 나면 pg_control 파일에 "여기까지 저장 완료"라는 북마크를 남깁니다. 이제 그 이전의 WAL 파일들은 복구 시 필요 없으므로 재활용하거나 삭제할 수 있습니다. 만약 시스템이 크래시되면, 마지막 Checkpoint 지점부터 WAL을 재생하면 됩니다. Checkpoint가 자주 일어나면 복구는 빠르지만 시스템 부하가 커지고, 드물게 일어나면 그 반대입니다. 이 균형을 맞추는 것이 튜닝의 핵심입니다.

![Fig 9.6: WAL 세그먼트 파일 관리](../docs/images/ch09/fig-9-06.png)
*WAL 세그먼트 파일 관리*

> **🔍 그림 해설**
>
> WAL 파일은 원형 테이프 레코더처럼 작동합니다. 파일 이름은 16진수로 000000010000000000000001, 000000010000000000000002 이렇게 순차적으로 증가합니다. 각 파일은 16MB 크기입니다. Checkpoint가 완료되면 PostgreSQL은 "이 파일들은 이제 안 봐도 돼"라고 판단한 오래된 WAL 파일을 재활용합니다. 삭제하는 게 아니라 이름을 바꿔서 다시 씁니다! 예를 들어 000000010000000000000001 파일을 다 쓰고 나면, 그걸 삭제하는 대신 000000010000000000000010으로 이름을 바꿔서 재사용합니다. 이렇게 하면 매번 파일을 새로 만드는 오버헤드를 피할 수 있습니다. max_wal_size 설정은 이 "테이프 전체 길이"를 정합니다. 1GB로 설정하면 약 64개(1GB / 16MB) 파일까지 유지하고, 그 이상 쌓이면 Checkpoint를 강제로 실행합니다. 복제나 아카이빙을 쓰면 WAL 파일이 삭제되지 않고 계속 쌓일 수 있으니 주의해야 합니다. pg_wal/ 디렉토리가 디스크를 가득 채우는 사고가 종종 일어나는 이유입니다.
...
![Fig 9.4: Full Page Writes 메커니즘](../docs/images/ch09/fig-9-04.png)
*Full Page Writes 메커니즘*

> **🔍 그림 해설**
>
> Full Page Writes(FPW)는 PostgreSQL의 안전장치입니다. 중요한 문서를 편집할 때를 상상해 보세요. Checkpoint 이후 처음으로 어떤 페이지를 수정할 때, PostgreSQL은 그 페이지 전체의 "복사본"을 WAL에 저장합니다. 왜 이렇게 할까요? 페이지는 8KB인데 디스크는 보통 512바이트나 4KB씩 씁니다. 만약 8KB를 쓰는 도중 정전이 되면 어떻게 될까요? 앞쪽 4KB는 새 내용, 뒤쪽 4KB는 옛날 내용으로 페이지가 반쪽짜리가 됩니다. 이것을 "찢어진 페이지(torn page)"라고 합니다. Full Page Write가 있으면 이런 일이 생겨도 안전합니다. WAL에 페이지 전체 복사본이 있으니, 복구할 때 그걸 그대로 복원하면 되거든요. 대신 WAL 크기가 늘어나는 단점이 있습니다. 특히 Checkpoint 직후에는 많은 페이지를 처음 수정하므로 WAL이 갑자기 커집니다. 하지만 같은 페이지를 또 수정할 때는 변경 부분만 기록하므로, 이후에는 WAL이 작아집니다. 데이터 안전성을 위한 필수적인 trade-off입니다.
